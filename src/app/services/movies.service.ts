...
export class MoviesService {
  ...
  fetchPopularMovies(page: number): Observable<IMoviesResponse> {
    return this.http.get<IMoviesResponse>(
      `${this.baseURL}/movie/popular?api_key=${this.API_KEY}&language=pt-BR&page=${page}`
    );
  }

  fetchAvailableMovies(): Observable<IMoviesResponse> {
    return this.http.get<IMoviesResponse>(
      `${this.baseURL}/movie/now_playing?api_key=${this.API_KEY}&language=pt-BR`
    );
  }

  fetchUpcomingMovies(): Observable<IMoviesResponse> {
    return this.http.get<IMoviesResponse>(
      `${this.baseURL}/movie/upcoming?api_key=${this.API_KEY}&language=pt-BR`
    );
  }

  fetchTopRatedMovies(): Observable<IMoviesResponse> {
    return this.http.get<IMoviesResponse>(
      `${this.baseURL}/movie/top_rated?api_key=${this.API_KEY}&language=pt-BR`
    );
  }

  fetchMoviesByQuery(query: string): Observable<IMoviesResponse> {
    return this.http.get<IMoviesResponse>(
      `${this.baseURL}/search/movie?api_key=${this.API_KEY}&language=pt-BR&query=${query}&include_adult=false`
    );
  }

  fetchMoviesByCategory(id: number): Observable<IMoviesResponse> {
    return this.http.get<IMoviesResponse>(
      `${this.baseURL}/discover/movie?api_key=${this.API_KEY}&language=pt-BR&include_adult=false&sort_by=popularity.desc&with_genres=${id}`
    );
  }

  fetchRecommendationsByMovieId(id: string): Observable<IMoviesResponse> {
    return this.http.get<IMoviesResponse>(
      `${this.baseURL}/movie/${id}/recommendations?api_key=${this.API_KEY}&language=pt-BR`
    );
  }

  fetchVideosByMovieId(id: number): Observable<IMovieVideoResponse> {
    return this.http.get<IMovieVideoResponse>(
      `${this.baseURL}/movie/${id}/videos?api_key=${this.API_KEY}&language=pt-BR`
    );
  }

  getMovieById(id: number): Observable<MovieDetails> {
    return this.http.get<MovieDetails>(
      `${this.baseURL}/movie/${+id}?api_key=${this.API_KEY}&language=pt-BR`
    );
  }
